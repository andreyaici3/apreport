<x-app-layout menuActive="modul">
    ...
    <div class="container-fluid">
                            ...
                            <div class="col-12">
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{ $nomor++ }}</td>
                                            <td>{{ $value->kode_modul }}</td>
                                            <td>{{ $value->nama_modul }}</td>
                                            <td>Rp. {{ number_format($value->sisa_saldo, 0, ',', '.') }}</td>
                                            <td>
                                                <a href="{{ route('atp.modul.edit', ['id' => $value->id]) }}" class="btn btn-xs btn-primary">
                                                    <i class="fas fa-edit"></i>
                                                    Edit
                                                </a>|
                                                <form id="delete{{ $value->id }}" action="{{ route('atp.modul.delete', ['id' => $value->id]) }}" style="display: inline-block;" method="POST">
                                                    @csrf
                                                    @method('delete')
        ...
        <div class="row mt-1">
            <div class="col-3">
                <a href="{{ route('atp.modul.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i>
                    MODUL</a>
            </div>
        </div>
    </div>
